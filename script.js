...
const openBtn = document.querySelector(".animate-bounce");
...
// Đóng popup khi ấn nút "Đóng"
closeBtn.addEventListener("click", () => {
  popup.classList.remove("show");
});

sendBtn.addEventListener('click', () => {
  const message = popupTextarea.value; // Lấy nội dung từ textarea (nếu cần thiết)
  console.log("Message: ", message);
  // Hiển thị popup cảm ơn từ SweetAlert2
  Swal.fire({
    icon: 'success',
    title: 'Cảm ơn bạn! 💖',
    text: 'Nội dung đã được gửi thành công!',
  }).then(() => {
    // Ẩn popup gốc và popup cảm ơn sau khi SweetAlert2 đóng
    popup.classList.add('hidden');
  });
});

const API_URL = 'http://localhost:2057/api/items'; // URL của API backend

// Lấy danh sách items từ API và hiển thị
...
// Lấy danh sách items khi tải trang
fetchItems();